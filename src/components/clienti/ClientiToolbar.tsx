"use client";

import { Search } from "lucide-react";
import { SORT_OPTIONS, STATO_FILTER_OPTIONS } from "@/lib/clienti/constants";
import type { ClienteStato } from "@/types/cliente";

type ClientiToolbarProps = {
  search: string;
  stato: ClienteStato | "tutti";
  sortValue: string;
  resultCount: number;
  onSearchChange: (value: string) => void;
  onStatoChange: (stato: ClienteStato | "tutti") => void;
  onSortChange: (sortValue: string) => void;
};

export function ClientiToolbar({
  search,
  stato,
  sortValue,
  resultCount,
  onSearchChange,
  onStatoChange,
  onSortChange,
}: ClientiToolbarProps) {
  return (
    <div className="mb-4 space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative max-w-md flex-1">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" />
          <input
            type="search"
            placeholder="Cerca per nome, email o azienda..."
            value={search}
            onChange={(event) => onSearchChange(event.target.value)}
            className="h-9 w-full rounded-md border border-zinc-200 bg-white pl-9 pr-3 text-sm text-zinc-900 placeholder:text-zinc-400 outline-none transition-colors focus:border-zinc-400 focus:ring-2 focus:ring-zinc-100"
          />
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <select
            value={sortValue}
            onChange={(event) => onSortChange(event.target.value)}
            className="h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm text-zinc-700 outline-none transition-colors focus:border-zinc-400 focus:ring-2 focus:ring-zinc-100"
            aria-label="Ordina clienti"
          >
            {SORT_OPTIONS.map((option) => (
              <option key={option.value} value={option.value}>
                {option.label}
              </option>
            ))}
          </select>

          <span className="text-sm text-zinc-500">
            {resultCount} {resultCount === 1 ? "risultato" : "risultati"}
          </span>
        </div>
      </div>

      <div className="flex flex-wrap gap-1.5">
        {STATO_FILTER_OPTIONS.map((option) => {
          const isActive = stato === option.value;
          return (
            <button
              key={option.value}
              type="button"
              onClick={() => onStatoChange(option.value)}
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                isActive
                  ? "bg-zinc-900 text-white"
                  : "bg-white text-zinc-600 ring-1 ring-zinc-200 hover:bg-zinc-50"
              }`}
            >
              {option.label}
            </button>
          );
        })}
      </div>
    </div>
  );
}
